export function Navbar() {
  const handleContactClick = () => {
    window.open("https://www.nutrient.io/contact-sales", "_blank");
  };

  return (
    <header className="flex items-center justify-between h-10 px-2 sm:px-4 bg-white border-b border-neutral-200">
      {/* Left: Brand */}
      <div className="flex items-baseline gap-1 font-['Inter'] min-w-0 flex-1">
        <span className="text-md sm:text-lg font-bold tracking-tight text-neutral-900 truncate">
          Nutrient
        </span>
        <span className="text-xs sm:text-xs font-medium text-neutral-600 truncate">
          WebSDK Demo
        </span>
      </div>

      {/* Right: CTA */}
      <div className="flex items-center ml-1">
        <div
          onClick={handleContactClick}
          className="cursor-pointer bg-neutral-800 text-white
                     text-xs xs:text-sm px-3 xs:px-4 py-1.5 h-7 xs:h-8
                     hover:bg-neutral-700 active:bg-neutral-800 transition-colors
                     flex items-center justify-center whitespace-nowrap select-none"
        >
          <span className="font-medium">Contact Sales</span>
        </div>
      </div>
    </header>
  );
}